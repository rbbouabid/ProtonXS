...
// ### daughter info ###
TH1D *hMCNDaughters  = new TH1D("hMCNDaughters", "Number of daughters", 10, 0, 10);
...
// ### step size distance ###
TH1D *hMCGeant4StepSize = new TH1D("hMCG4StepSize", "Geant 4 Step Size",    40,  0, 40);
TH1D *hMCGeant4dEdX     = new TH1D("hMCG4dEdX",     "Geant 4 dE/dX (hack)", 100, 0, 10);
TH1D *hRecodEdX         = new TH1D("hRecodEdX",     "Reco dE/dX",           100, 0, 10);
...
TH2D *hRecoMatchedElasticAngleCmpsn         = new TH2D("hRecoMatchedElasticAngleCmpsn",   "Reconstructed vs True Elastic Angle", 40, 0, 20, 40, 0, 20);
TH1D *hRecoDeltaEnergy                      = new TH1D("hRecoDeltaEnergy", "Initial KE Minus Total KE Loss (MeV)"                   , 50, 0, 1000);
TH1D *hRecoDeltaEnergyChannel0              = new TH1D("hRecoDeltaEnergyChannel0", "Initial KE Minus Total KE Loss (MeV) Channel 0"      , 50, 0, 1000);
TH2D *hRecoDeltaEnergyChannel0vsKEinit      = new TH2D("hRecoDeltaEnergyChannel0vsKEinit", "Initial KE Minus Total KE Loss Channel 0 vs Initial KE", 50, 0, 1000, 50, 0, 1000);
TH2D *hRecoDeltaEnergyvsIntKE               = new TH2D("hRecoDeltaEnergyvsIntKE", "Delta KE vs Int KE"                 , 50, 0, 1000, 50, 0, 1000);
...
TH1D *hRecoEndMeandedx                      = new TH1D("hRecoEndMeandedx", "End Mean dE/dX for straight tracks"       , 40, 0, 40);
TH1D *hRecoEndMeandedxElastic               = new TH1D("hRecoEndMeandedxElastic", "End Mean dE/dX for straight tracks (elastics)", 40, 0, 40);
TH1D *hRecoEndMeandedxInelast               = new TH1D("hRecoEndMeandedxInelast", "End Mean dE/dX for straight tracks (inelasts)", 40, 0, 40);
TH1D *hRecoEndMeandedxNeither               = new TH1D("hRecoEndMeandedxNeither", "End Mean dE/dX for straight tracks (neithers)", 40, 0, 40);
...
TH1D *hRecoInteractingKE        = new TH1D("hRecoInteractingKE", "Interacting Kinetic Energy", 20, 0, 1000);
    ...
    TH1D *hRecoInteractingKETrueSignal        = new TH1D("hRecoInteractingKETrueSignal", "Interacting Kinetic Energy True Signal" , 20, 0, 1000);
        TH1D *hRecoInteractingKETrueSignalInelast     = new TH1D("hRecoInteractingKETrueSignalInelast",   "Interacting KE True Signal Inelastic"     , 20, 0, 1000);
        ...
        TH2D *hRecoInteractingKETrueSignalEA = new TH2D("hRecoInteractingKETrueSignalEA", "Interacting KE True Signal Elastic vs Angle", 500, 0, 1000, 40, 0, 20);
        TH2D *hRecoInteractingKETrueSignalIA = new TH2D("hRecoInteractingKETrueSignalIA", "Interacting KE True Signal Inelast vs Angle", 500, 0, 1000, 40, 0, 20);
...
// ## by channel ##
TH1D *hRecoResolution0to200        = new TH1D("hRecoResolution0to200",        "Reco Interacting KE for True Slice (0, 200)"       ,   125 , -200 , 500);
TH1D *hRecoResolution0to200Chan1   = new TH1D("hRecoResolution0to200Chan1",   "Reco Interacting KE for True Slice (0, 200) Chan1" ,   125 , -200 , 500);
TH1D *hRecoResolution0to200Chan2   = new TH1D("hRecoResolution0to200Chan2",   "Reco Interacting KE for True Slice (0, 200) Chan2" ,   125 , -200 , 500);
TH1D *hRecoResolution0to200Chan3   = new TH1D("hRecoResolution0to200Chan3",   "Reco Interacting KE for True Slice (0, 200) Chan3" ,   125 , -200 , 500);
TH1D *hRecoResolution0to200Chan4   = new TH1D("hRecoResolution0to200Chan4",   "Reco Interacting KE for True Slice (0, 200) Chan4" ,   125 , -200 , 500);
...
for (Long64_t jentry=0; jentry<50000;jentry++) {
   ...
   // check for protons reaching tpc
  ...
  for(size_t nG4 = 0; nG4 < geant_list_size; nG4++)
    ...
    double dX = 0; double dE = 0; double dEdX = 1.;
    for(size_t npt = 1; npt < (*NTrTrajPts)[0]-1; npt++)
      ...
      if(is_in_tpc(npt))
        ...
        if(tpc_spt_cnt == 0)
          ...
          //std::cout << "        first true KE inside TPC: " << temp_currentKE << std::endl;
        ...
        dX += g4dp;
        //std::cout << "distance to last space point: " << g4dp << std::endl; 
        ...
        if(dX > 5)
          {
          dEdX = dE / dX;
          hMCGeant4dEdX->Fill(dEdX);
          dX = 0; dE = 0; 
          }
        //std::cout << "midke: " << midke << std::endl;
...
// ### reco info ###
      ...
      double temp_init_ke   = sqrt( pow(momentum, 2) + pow(mass, 2) ) - mass;
      double interaction_ke = sqrt( pow(interaction_p, 2) + pow(mass, 2) ) - mass;
      //double ke = 1.;
      //std::cout << "    INIT KINETIC ENERGY : " << ke << std::endl;
      // ### reco track loop ###
        ...
        if(ntracks_reco == 1)
          ...
          for(int i = 0; i < 10; i++)
            ...
            if(dummy_angle_array[i] != -1)
              {
              // ## checking g4 thinks there was an interaciton
              if(did_interact == true && (*InteractionPointType)[0] != 8)
                {
          ...
          // ### probably where I need to collect 1 track topology calo info ###
          ...
          if(dummy_spt_array[5] == -1)
            ...
            if(end_dedx_mean >= 13 || straight_punchthru == true)
              ...
              if((*InteractionPointType)[0] == 0)
                {
                //std::cout << "reco total lost: " << reco_total_lost << std::endl;
                //std::cout << "initial ke: " << temp_init_ke << std::endl; 
...
std::cout << "double checking area" << std::endl;
...
for(auto const& tuple: matched_ke_tuple) 
    ...
    if(true_int_ke < 200)
      {
      hRecoResolution0to200->Fill(reco_int_ke);
      if(topology_type == 1){hRecoResolution0to200Chan1->Fill(reco_int_ke);}
      if(topology_type == 2){hRecoResolution0to200Chan2->Fill(reco_int_ke);}
      if(topology_type == 3){hRecoResolution0to200Chan3->Fill(reco_int_ke);}
      if(topology_type == 4){hRecoResolution0to200Chan4->Fill(reco_int_ke);}
      }
...
//// ###############################################
TFile myfile("mcscratch_histos.root","RECREATE");
...
hRecoResolution0to200->Write();
hRecoResolution0to200Chan1->Write();
hRecoResolution0to200Chan2->Write();
hRecoResolution0to200Chan3->Write();
hRecoResolution0to200Chan4->Write();
